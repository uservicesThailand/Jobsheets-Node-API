const service = require("./insulation.service");
const resUtil = require("../../../../utils/response.util");
const { mapResponse } = require("./insulation.serializer");

const save = async (req, res) => {
  try {
    const { inspNo } = req.params;

    const result = await service.upsert(inspNo, req.userKey, req.body.data);
    if (!result.success) {
